...
public class EmployeeDAOHibernateImpl implements EmployeeDAO {
	...
	private boolean saveOrUpdate(EmployeeInfoBean empBean) {
		...
		for (EmployeeEducationInfoBean employeeEducationInfoBean : empBean.getEducationInfoBean()) {
			employeeEducationInfoBean.getEmpEducationInfoPKBean().setInfoBean(empBean);
		}

		for (EmployeeAddressInfoBean addressInfoBean : empBean.getAddressInfoBean()) {
			addressInfoBean.getEmpAddressPKBean().setInfoBean(empBean);
		}

		for (EmployeeExperienceInfoBean employeeExperienceInfoBean : empBean.getExpirenceInfoBean()) {
			employeeExperienceInfoBean.getEmpExperienceInfoPKBean().setInfoBean(empBean);
		}

		EmployeeOtherInfoBean otherInfoBean = empBean.getOtherInfoBean();
		otherInfoBean.setInfoBean(empBean);
